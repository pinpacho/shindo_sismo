...
function epochToDateTime(epochTime) {
  var epochDate = epochToJsDate(epochTime);
  var localDate = new Date(epochDate.toLocaleString("en-US", { timeZone: "America/Bogota" }));

  var dateTime = localDate.getFullYear() + "/" +
      ("00" + (localDate.getMonth() + 1)).slice(-2) + "/" +
      ("00" + localDate.getDate()).slice(-2) + " " +
      ("00" + localDate.getHours()).slice(-2) + ":" +
      ("00" + localDate.getMinutes()).slice(-2) + ":" +
      ("00" + localDate.getSeconds()).slice(-2);

  return dateTime;
}

function plotValues(chart, timestamp, value) {
  var localDate = epochToJsDate(timestamp);
  var timezoneOffset = localDate.getTimezoneOffset() * 60000; // Offset in milliseconds
  var x = localDate.getTime() - timezoneOffset;
  var y = Number(value);
  ...
  const IfFmaxElement = document.getElementById("IfFmax");
    ...
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          var jsonData = snapshot.toJSON(); // example: {Acceleration: 25.02, IfF: 50.20, IfFmax: 1008.48, timestamp:1641317355}
          // Save values on variables

          var IfFmax = jsonData.IfFmax;
          var timestamp = jsonData.timestamp;
          // Plot the values on the charts

          plotValues(chartP, timestamp, IfFmax);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        IfFmaxElement.innerHTML = IfFmax;
