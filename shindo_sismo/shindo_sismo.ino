...
#define LAST_BUFFERSIZE 30 /* Encuentra la aceleración en los ultimos 3sec , encuentra af que sigue por 0.3sec ,
                            asumiendo loopforSeismo() loops en 10ms*/
//#define SIMULATION
//#define BCJL1 //use BCJ-L1 wave , else BCJ-L2 is selected
#ifdef SIMULATION
#define BCJL1_SIZE 6000 //10msec sampling 60sec
#define BCJL2_SIZE 12000 //10msec sampling 120sec
...
#define MAX1G 16384.0 // 1G en 2Gmax 
#define GRAVITY 980.0 // 1G en cm/sec^2
...
FilterOnePole XFHigh(HIGHPASS, 0.2), YFHigh(HIGHPASS, 0.2), ZFHigh(HIGHPASS, 0.2);//0.2Hz filtro
...
// Función que obtiene la época actual.
...
//  Wire.setClock(400000L);
  ...
  //El bus I2C es compartido por dos dispositivos, semaI2C toma el control exclusivo del bus I2C
  semaI2C = xSemaphoreCreateMutex();
  xSemaphoreGive( semaI2C );  /* Sede un semaforo */

   delay(3000); //Espera para estabilizar el MPU650
  if (initMPU6050()==-1) {
  ...
  // Obteniendo el UID del usuario,podria demorar
  ...
  clearLastBuffer();//LastBuffer se utiliza para encontrar una aceleración superior a 0.3sec
  ...
  if (xQueue == NULL)
  {
    ...
    delay(3000);//Publica despues de que el sensor se estabilice
...
int initMPU6050() {
  ...
  writeMPU6050(MPU6050_ACCEL_CONFIG, MPU6050_AFS_SEL_2G); //  ±2g
...
//shared variables
float af2 = 0.0;
float IfF = 0.0;  //Indice sismico
float IfF_h = 0.0; //Indice sismico para la aceleración horizontal
float IfFtemp = 0.0;
float IfFmax = 0.0; //max IfF durante el periodo de observación
float IfF_htemp = 0.0;
float IfF_hmax = 0.0;//max IfF_h durante el periodo de observación
...
void codeForSeismo( void * parameter)
{
  ...
  while (1) {
    ...
    loopforSeismo(); //Actualiza IfF IfF_h cada 100msec
    //hold max IfF,IfF_h
    if ( IfF > IfFtemp )
    {
      IfFtemp = IfF;
...
void loopforSeismo() {
  ...
  portTickType xLastWakeTime;
  xLastWakeTime = xTaskGetTickCount();// Inicialice la variable xLastWakeTime con la hora actual.
...
#define REMOVE_GRAVITY
  ...
  float af_h = GRAVITY * sqrt(XFHigh.output() * XFHigh.output() + YFHigh.output() * YFHigh.output() ); //en gal
  float af = GRAVITY * sqrt(XFHigh.output() * XFHigh.output() + YFHigh.output() * YFHigh.output() + ZFHigh.output() * ZFHigh.output() ); //en gal
#else
  // without soft high pass filter 1Hz
  float af_h = GRAVITY * sqrt( (float)AcX * (float)AcX / MAX1G / MAX1G + (float)AcY * (float)AcY / MAX1G / MAX1G  ); //en gal
  float af = GRAVITY * sqrt( (float)AcX * (float)AcX / MAX1G / MAX1G + (float)AcY * (float)AcY / MAX1G / MAX1G + (float)AcZ * (float)AcZ / MAX1G / MAX1G ); //en gal
#endif
  ...
  //Encuentra el minimo en las ultimas 3 lecturas , que corresponde a 0.3sec
...
//#define DEBUG
  ...
  IfF = temp_last[2]; // aceleración al menos 0.3sec
  ...
  //Serial.println("  IfF, Iftemp, Ifmax");
